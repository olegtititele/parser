...
async def echo(call: types.CallbackQuery, state: FSMContext):
	...
	if check_sub_channel(await bot.get_chat_member(chat_id=cf.CHANNEL_CHAT_ID, user_id=call.from_user.id)):
		...
		if call.data == "back_to_menu":
			...
			await state.finish()
			await bot.send_photo(chat_id=message.from_user.id, photo=photo, caption=f"🆔 <b>Ваш ид:</b> <code>{call.from_user.id}</code>", parse_mode=types.ParseMode.HTML, reply_markup=main_kb)	
